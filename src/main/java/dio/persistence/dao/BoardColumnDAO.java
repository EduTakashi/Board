...
public class BoardColumnDAO {
    ...
    public List<BoardColumnDTO> findByBoardIdDetailed(final Long boardId) throws SQLException {
        ...
        var sql =
                """
                SELECT bc.id, bc.name,bc.kind, (SELECT COUNT(c.id) FROM cards c WHERE c.board_column_id = bc.id) AS cards_amount FROM boards_columns bc WHERE board_id = ? ORDER BY `order`;
                """;
        try(var statement = connection.prepareStatement(sql)){
            statement.setLong(1, boardId);
            statement.executeQuery();
